...
(function () {
  ...
  const makeTab = _info => {
    const items = [{
      type: 'htmlpanel',
      html: '<div id="upload_container" class="text-xs">Upload container</div>'
    }];
    return {
      title: 'Upload',
      name: 'upload',
  ...
  const register = editor => {
    editor.ui.registry.addToggleButton('image_browser', {
      icon: 'image',
    ...
    editor.ui.registry.addMenuItem('image_browser', {
      icon: 'image',
      text: 'Image...',
      onAction: Dialog(editor).open
    });
    editor.ui.registry.addContextMenu('image_browser', { update: element => isFigure(element) || isImage(element) && !isPlaceholderImage(element) ? ['image'] : [] });
  };

  var Plugin = () => {
    global$4.add('image_browser', editor => {
      register$2(editor);
      setup(editor);
      register(editor);
      register$1(editor);
